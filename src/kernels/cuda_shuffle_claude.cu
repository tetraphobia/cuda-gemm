...
__global__ void _shfl_gemm_kernel(const float *A, const float *B, float *C,
  ...
  const int warpColBase  = blockCol + colWarp * WARP_SIZE;  // independent of rowWarp

  __shared__ float As[TILE_M][TILE_K];

  float acc[CHUNK_SIZE];
  #pragma unroll
  for (int i = 0; i < CHUNK_SIZE; ++i) acc[i] = 0.0f;
...
void multiply_shuffle_claude(const float *A, const float *B, float *C,
